...
@pytest.mark.asyncio
async def test_default_configuration(temp_storage_path):
    """Test FileStore initializes with default configuration"""
    store = await FileStore.create(base_path=str(temp_storage_path))
...
@pytest.mark.asyncio
async def test_env_var_configuration(temp_storage_path, monkeypatch):
    """Test FileStore respects environment variables"""
    # Set environment variables
    monkeypatch.setenv('NARRATOR_MAX_FILE_SIZE', '1048576')  # 1MB
    monkeypatch.setenv('NARRATOR_MAX_STORAGE_SIZE', '10485760')  # 10MB
...
@pytest.mark.asyncio
async def test_invalid_env_vars_use_defaults(temp_storage_path, monkeypatch):
    """Test FileStore handles invalid environment variables gracefully"""
    # Set invalid environment variables
    monkeypatch.setenv('NARRATOR_MAX_FILE_SIZE', 'invalid')
    monkeypatch.setenv('NARRATOR_MAX_STORAGE_SIZE', 'invalid')
...
@pytest.mark.asyncio
async def test_constructor_overrides_env_vars(temp_storage_path, monkeypatch):
...
@pytest.mark.asyncio
async def test_env_var_mime_types(temp_storage_path, monkeypatch):
    """Test FileStore respects MIME type environment variables"""
    # Set environment variable
    monkeypatch.setenv('NARRATOR_ALLOWED_MIME_TYPES', 'image/jpeg,image/png,application/pdf')
    
    store = await FileStore.create(base_path=str(temp_storage_path))
    assert store.allowed_mime_types == {'image/jpeg', 'image/png', 'application/pdf'}

@pytest.mark.asyncio
async def test_invalid_mime_types_use_defaults(temp_storage_path, monkeypatch):
    """Test FileStore handles invalid MIME types gracefully"""
    # Test with invalid format
    monkeypatch.setenv('NARRATOR_ALLOWED_MIME_TYPES', 'invalid,image/png,not-a-mime-type')
    
    store = await FileStore.create(base_path=str(temp_storage_path))
    assert store.allowed_mime_types == FileStore.DEFAULT_ALLOWED_MIME_TYPES

@pytest.mark.asyncio
async def test_mime_types_constructor_override(temp_storage_path, monkeypatch):
    """Test constructor MIME types override environment variables"""
    # Set environment variable
    monkeypatch.setenv('NARRATOR_ALLOWED_MIME_TYPES', 'image/jpeg,image/png')
...
@pytest.mark.asyncio
async def test_get_file_url(temp_store: FileStore):
    """Test getting file URLs."""
    content = b'Test content for URL'
    result = await temp_store.save(content=content, filename='test.txt')
    
    # Test URL generation
    url = temp_store.get_file_url(result['storage_path'])
    assert url is not None
    assert result['storage_path'] in url

@pytest.mark.asyncio
async def test_storage_backend_type(temp_store: FileStore):
    """Test storage backend type is correctly set."""
    assert temp_store.storage_backend == "local"

@pytest.mark.asyncio
async def test_sharded_storage_structure(temp_store: FileStore):
    """Test that files are stored in sharded directory structure."""
    content = b'Test sharded storage'
    result = await temp_store.save(content=content, filename='test.txt')
    
    # Storage path should contain sharded directory structure
    storage_path = result['storage_path']
    assert '/' in storage_path  # Should have directory separator
    
    # Verify file exists at the sharded path
    full_path = temp_store.base_path / storage_path
    assert full_path.exists()
    assert full_path.is_file()

@pytest.mark.asyncio 
async def test_mime_type_detection(temp_store: FileStore):
    """Test automatic MIME type detection."""
    # Test with text content
    text_content = b'This is plain text'
    result = await temp_store.save(content=text_content, filename='auto.txt')
    assert result['mime_type'] == 'text/plain'
    
    # Test with explicit MIME type override
    json_content = b'{"key": "value"}'
    result = await temp_store.save(content=json_content, filename='data.json', mime_type='application/json')
